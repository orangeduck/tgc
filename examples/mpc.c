...
static mpc_input_t *mpc_input_new_string(const char *filename, const char *string) {
  ...
  mpc_input_t *i = tgc_alloc(&gc, sizeof(mpc_input_t));
  
  i->filename = tgc_alloc(&gc, strlen(filename) + 1);
  strcpy(i->filename, filename);
  i->type = MPC_INPUT_STRING;
  
  i->state = mpc_state_new();
  
  i->string = tgc_alloc(&gc, strlen(string) + 1);
  ...
  i->marks = tgc_alloc(&gc, sizeof(mpc_state_t) * i->marks_slots);
  i->lasts = tgc_alloc(&gc, sizeof(char) * i->marks_slots);
  i->last = '\0';
...
static void *mpc_malloc(mpc_input_t *i, size_t n) {
  ...
  if (n > sizeof(mpc_mem_t)) { return tgc_alloc(&gc, n); }
...
static void *mpc_realloc(mpc_input_t *i, void *p, size_t n) {
  ...
  if (!mpc_mem_ptr(i, p)) { return tgc_realloc(&gc, p, n); }
...
static void *mpc_export(mpc_input_t *i, void *p) {
  ...
  q = tgc_alloc(&gc, sizeof(mpc_mem_t));
  memcpy(q, p, sizeof(mpc_mem_t));
  mpc_free(i, p);
  return q; 
}
...
static void mpc_input_mark(mpc_input_t *i) {
  ...
  if (i->marks_num > i->marks_slots) {
    ...
    i->marks = tgc_realloc(&gc, i->marks, sizeof(mpc_state_t) * i->marks_slots);
    i->lasts = tgc_realloc(&gc, i->lasts, sizeof(char) * i->marks_slots);
  }
...
static int mpc_input_success(mpc_input_t *i, char c, char **o) {
  ...
  &&  i->buffer && !mpc_input_buffer_in_range(i)) {
    i->buffer = tgc_realloc(&gc, i->buffer, strlen(i->buffer) + 2);
...
char *mpc_err_string(mpc_err_t *x) {
  ...
  int pos = 0; 
  int max = 1023;
  char *buffer = tgc_calloc(&gc, 1, 1024);
  ...
  return tgc_realloc(&gc, buffer, strlen(buffer) + 1);
}
...
static mpc_err_t *mpc_err_file(const char *filename, const char *failure) {
  mpc_err_t *x;
  x = tgc_alloc(&gc, sizeof(mpc_err_t));
  x->filename = tgc_alloc(&gc, strlen(filename) + 1);
  strcpy(x->filename, filename);
  x->state = mpc_state_new();
  x->expected_num = 0;
  x->expected = NULL;
  x->failure = tgc_alloc(&gc, strlen(failure) + 1);
  strcpy(x->failure, failure);
  x->recieved = ' ';
  return x;
}
...
static void mpc_parse_dtor(mpc_input_t *i, mpc_dtor_t d, mpc_val_t *x) {
  if (d == free || d == nothing) { mpc_free(i, x); return; }
  d(mpc_export(i, x));
}
...
static void mpc_undefine_unretained(mpc_parser_t *p, int force) {
  ...
  switch (p->type) {
    ...
    case MPC_TYPE_FAIL: //free(p->data.fail.m);
    break;
...
static mpc_parser_t *mpc_undefined(void) {
  mpc_parser_t *p = tgc_calloc(&gc, 1, sizeof(mpc_parser_t));
  p->retained = 0;
...
mpc_parser_t *mpc_new(const char *name) {
  ...
  p->retained = 1;
  p->name = tgc_realloc(&gc, p->name, strlen(name) + 1);
  strcpy(p->name, name);
  return p;
}
...
mpc_parser_t *mpc_copy(mpc_parser_t *a) {
  ...
  if (a->name) {
    p->name = tgc_alloc(&gc, strlen(a->name)+1);
  ...
  switch (a->type) {
    ...
    case MPC_TYPE_FAIL:
      p->data.fail.m = tgc_alloc(&gc, strlen(a->data.fail.m)+1);
    ...
    case MPC_TYPE_NONEOF:
    case MPC_TYPE_STRING:
      p->data.string.x = tgc_alloc(&gc, strlen(a->data.string.x)+1);
    ...
    case MPC_TYPE_EXPECT:
      ...
      p->data.expect.m = tgc_alloc(&gc, strlen(a->data.expect.m)+1);
    ...
    case MPC_TYPE_OR:
      p->data.or.xs = tgc_alloc(&gc, a->data.or.n * sizeof(mpc_parser_t*));
      for (i = 0; i < a->data.or.n; i++) {
        p->data.or.xs[i] = mpc_copy(a->data.or.xs[i]);
      }
    break;
    case MPC_TYPE_AND:
      p->data.and.xs = tgc_alloc(&gc, a->data.and.n * sizeof(mpc_parser_t*));
      for (i = 0; i < a->data.and.n; i++) {
        p->data.and.xs[i] = mpc_copy(a->data.and.xs[i]);
      }
      p->data.and.dxs = tgc_alloc(&gc, (a->data.and.n-1) * sizeof(mpc_dtor_t));
...
void mpc_cleanup(int n, ...) {
  int i;
  mpc_parser_t **list = tgc_alloc(&gc, sizeof(mpc_parser_t*) * n);
...
mpc_parser_t *mpc_fail(const char *m) {
  ...
  p->data.fail.m = tgc_alloc(&gc, strlen(m) + 1);
...
mpc_parser_t *mpc_failf(const char *fmt, ...) {
  ...
  va_start(va, fmt);
  buffer = tgc_alloc(&gc, 2048);
  vsprintf(buffer, fmt, va);
  va_end(va);
  
  buffer = tgc_realloc(&gc, buffer, strlen(buffer) + 1);
...
mpc_parser_t *mpc_expect(mpc_parser_t *a, const char *expected) {
  ...
  p->data.expect.x = a;
  p->data.expect.m = tgc_alloc(&gc, strlen(expected) + 1);
...
mpc_parser_t *mpc_oneof(const char *s) {
  ...
  p->data.string.x = tgc_alloc(&gc, strlen(s) + 1);
...
mpc_parser_t *mpc_or(int n, ...) {
  ...
  p->type = MPC_TYPE_OR;
  p->data.or.n = n;
  p->data.or.xs = tgc_alloc(&gc, sizeof(mpc_parser_t*) * n);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  p->data.and.n = n;
  p->data.and.f = f;
  p->data.and.xs = tgc_alloc(&gc, sizeof(mpc_parser_t*) * n);
  p->data.and.dxs = tgc_alloc(&gc, sizeof(mpc_dtor_t) * (n-1));
...
mpc_parser_t *mpc_escape(void) { return mpc_and(2, mpcf_strfold, mpc_char('\\'), mpc_any(), nothing); }
...
mpc_parser_t *mpc_real(void) {
  ...
  p1 = mpc_digits();
  p2 = mpc_maybe_lift(mpc_and(2, mpcf_strfold, mpc_char('.'), mpc_digits(), nothing), mpcf_ctor_str);
  p30 = mpc_oneof("eE");
  p31 = mpc_maybe_lift(mpc_oneof("+-"), mpcf_ctor_str);
  p32 = mpc_digits();
  p3 = mpc_maybe_lift(mpc_and(3, mpcf_strfold, p30, p31, p32, nothing, nothing), mpcf_ctor_str);
  
  return mpc_expect(mpc_and(4, mpcf_strfold, p0, p1, p2, p3, nothing, nothing, nothing), "real");
...
mpc_parser_t *mpc_char_lit(void) {
  return mpc_expect(mpc_between(mpc_or(2, mpc_escape(), mpc_any()), nothing, "'", "'"), "char");
}

mpc_parser_t *mpc_string_lit(void) {
  mpc_parser_t *strchar = mpc_or(2, mpc_escape(), mpc_noneof("\""));
  return mpc_expect(mpc_between(mpc_many(mpcf_strfold, strchar), nothing, "\"", "\""), "string");
}

mpc_parser_t *mpc_regex_lit(void) {  
  mpc_parser_t *regexchar = mpc_or(2, mpc_escape(), mpc_noneof("/"));
  return mpc_expect(mpc_between(mpc_many(mpcf_strfold, regexchar), nothing, "/", "/"), "regex");
}
...
mpc_parser_t *mpc_ident(void) {
  ...
  return mpc_and(2, mpcf_strfold, p0, p1, nothing);
}
...
static mpc_val_t *mpcf_re_and(int n, mpc_val_t **xs) {
  ...
  for (i = 0; i < n; i++) {
    p = mpc_and(2, mpcf_strfold, p, xs[i], nothing);
  }
  return p;
}
...
static mpc_val_t *mpcf_re_repeat(int n, mpc_val_t **xs) {
  ...
  return mpc_count(num, mpcf_strfold, xs[0], nothing);
}
...
static mpc_parser_t *mpc_re_escape_char(char c) {
  switch (c) {
    ...
    case 'b': return mpc_and(2, mpcf_snd, mpc_boundary(), mpc_lift(mpcf_ctor_str), nothing);
    case 'B': return mpc_not_lift(mpc_boundary(), nothing, mpcf_ctor_str);
    case 'A': return mpc_and(2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str), nothing);
    case 'Z': return mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str), nothing);
    case 'd': return mpc_digit();
    case 'D': return mpc_not_lift(mpc_digit(), nothing, mpcf_ctor_str);
    case 's': return mpc_whitespace();
    case 'S': return mpc_not_lift(mpc_whitespace(), nothing, mpcf_ctor_str);
    case 'w': return mpc_alphanum();
    case 'W': return mpc_not_lift(mpc_alphanum(), nothing, mpcf_ctor_str);
    default: return NULL;
  }
}
...
static mpc_val_t *mpcf_re_escape(mpc_val_t *x) {
  ...
  if (s[0] == '.') { //free(s);
    return mpc_any(); }
  if (s[0] == '^') { //free(s);
    return mpc_and(2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str), nothing); }
  if (s[0] == '$') { //free(s);
    return mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str), nothing); }
...
static mpc_val_t *mpcf_re_range(mpc_val_t *x) {
  ...
  for (i = comp; i < strlen(s); i++){
    ...
    if (s[i] == '\\') {
      ...
      if (tmp != NULL) {
        range = tgc_realloc(&gc, range, strlen(range) + strlen(tmp) + 1);
        strcat(range, tmp);
      } else {
        range = tgc_realloc(&gc, range, strlen(range) + 1 + 1);
    ...
    /* Regex Range...Range */
      ...
      if (s[i+1] == '\0' || i == 0) {
          range = tgc_realloc(&gc, range, strlen(range) + strlen("-") + 1);
          strcat(range, "-");
      } else {
        start = s[i-1]+1;
        end = s[i+1]-1;
        for (j = start; j <= end; j++) {
          range = tgc_realloc(&gc, range, strlen(range) + 1 + 1 + 1);
          range[strlen(range) + 1] = '\0';
...
mpc_parser_t *mpc_re(const char *re) {
  ...
  mpc_define(Regex, mpc_and(2, mpcf_re_or,
    Term, 
    mpc_maybe(mpc_and(2, mpcf_snd_free, mpc_char('|'), Regex, nothing)),
...
mpc_val_t *mpcf_ctor_str(void) { return tgc_calloc(&gc, 1, 1); }
mpc_val_t *mpcf_free(mpc_val_t *x) { //free(x);
  return NULL; }

mpc_val_t *mpcf_int(mpc_val_t *x) {
  int *y = tgc_alloc(&gc, sizeof(int));
  *y = strtol(x, NULL, 10);
  //free(x);
  return y;
}
...
mpc_val_t *mpcf_float(mpc_val_t *x) {
  float *y = tgc_alloc(&gc, sizeof(float));
  *y = strtod(x, NULL);
  //free(x);
  return y;
}
...
static mpc_val_t *mpcf_escape_new(mpc_val_t *x, const char *input, const char **output) {
  ...
  while (*s) {
    ...
    while (output[i]) {
      if (*s == input[i]) {
        y = tgc_realloc(&gc, y, strlen(y) + strlen(output[i]) + 1);
        strcat(y, output[i]);
        found = 1;
        break;
      }
...
static mpc_val_t *mpcf_unescape_new(mpc_val_t *x, const char *input, const char **output) {
  ...
  while (s[j]) {
    ...
    while (output[i]) {
      if (s[j+0] == output[i][0]
      &&  s[j+1] == output[i][1]) {
        y = tgc_realloc(&gc, y, strlen(y) + 1 + 1);
...
mpc_val_t *mpcf_strfold(int n, mpc_val_t **xs) {
  ...
  if (n == 0) { return tgc_calloc(&gc, 1, 1); }
  
  for (i = 0; i < n; i++) { l += strlen(xs[i]); }
  
  xs[0] = tgc_realloc(&gc, xs[0], l + 1);
...
mpc_ast_t *mpc_ast_new(const char *tag, const char *contents) {
  ...
  mpc_ast_t *a = tgc_alloc(&gc, sizeof(mpc_ast_t));
  
  a->tag = tgc_alloc(&gc, strlen(tag) + 1);
  strcpy(a->tag, tag);
  
  a->contents = tgc_alloc(&gc, strlen(contents) + 1);
...
mpc_ast_t *mpc_ast_add_child(mpc_ast_t *r, mpc_ast_t *a) {
  r->children_num++;
  r->children = tgc_realloc(&gc, r->children, sizeof(mpc_ast_t*) * r->children_num);
...
mpc_ast_t *mpc_ast_add_tag(mpc_ast_t *a, const char *t) {
  if (a == NULL) { return a; }
  a->tag = tgc_realloc(&gc, a->tag, strlen(t) + 1 + strlen(a->tag) + 1);
...
mpc_ast_t *mpc_ast_tag(mpc_ast_t *a, const char *t) {
  a->tag = tgc_realloc(&gc, a->tag, strlen(t) + 1);
  strcpy(a->tag, t);
  return a;
}
...
mpc_parser_t *mpca_state(mpc_parser_t *a) {
  return mpc_and(2, mpcf_state_ast, mpc_state(), a, nothing);
}
...
static mpc_parser_t *mpca_grammar_find_parser(char *x, mpca_grammar_st_t *st) {
  ...
  if (is_number(x)) {
    ...
    while (st->parsers_num <= i) {
      st->parsers_num++;
      st->parsers = tgc_realloc(&gc, st->parsers, sizeof(mpc_parser_t*) * st->parsers_num);
...
mpc_parser_t *mpca_grammar_st(const char *grammar, mpca_grammar_st_t *st) {
  ...
  mpc_define(Grammar, mpc_and(2, mpcaf_grammar_or,
    Term,
    mpc_maybe(mpc_and(2, mpcf_snd_free, mpc_sym("|"), Grammar, nothing)),
    mpc_soft_delete
  ));
  ...
  mpc_define(Factor, mpc_and(2, mpcaf_grammar_repeat,
    Base,
      mpc_or(6,
        ...
        mpc_sym("?"),
        mpc_sym("!"),
        mpc_tok_brackets(mpc_int(), nothing),
        mpc_pass()),
    mpc_soft_delete
  ));
  ...
  mpc_define(Base, mpc_or(5,
    ...
    mpc_apply_to(mpc_tok_braces(mpc_or(2, mpc_digits(), mpc_ident()), nothing), mpcaf_grammar_id, st),
...
static mpc_val_t *mpca_stmt_afold(int n, mpc_val_t **xs) {
  mpca_stmt_t *stmt = tgc_alloc(&gc, sizeof(mpca_stmt_t));
...
static mpc_val_t *mpca_stmt_fold(int n, mpc_val_t **xs) {
  ...
  int i;
  mpca_stmt_t **stmts = tgc_alloc(&gc, sizeof(mpca_stmt_t*) * (n+1));
...
static mpc_err_t *mpca_lang_st(mpc_input_t *i, mpca_grammar_st_t *st) {
  ...
  mpc_define(Stmt, mpc_and(5, mpca_stmt_afold,
    ...
    nothing, nothing, nothing, mpc_soft_delete
  ));
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
  ...
  while (1) {
    ...
    && !p->data.or.xs[p->data.or.n-1]->retained) {
      ...
      p->data.or.n = n + m - 1;
      p->data.or.xs = tgc_realloc(&gc, p->data.or.xs, sizeof(mpc_parser_t*) * (n + m -1));
    ...
    &&  p->data.and.xs[0]->data.and.f == mpcf_fold_ast) {
      ...
      p->data.and.n = n + m - 1;
      p->data.and.xs = tgc_realloc(&gc, p->data.and.xs, sizeof(mpc_parser_t*) * (n + m - 1));
      p->data.and.dxs = tgc_realloc(&gc, p->data.and.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
    ...
    &&  p->data.and.xs[0]->data.and.f == mpcf_strfold) {
      ...
      memmove(p->data.and.xs + m, p->data.and.xs + 1, (n - 1) * sizeof(mpc_parser_t*));
      memmove(p->data.and.xs, t->data.and.xs, m * sizeof(mpc_parser_t*));
      for (i = 0; i < p->data.and.n-1; i++) { p->data.and.dxs[i] = nothing; }
      //free(t->data.and.xs); free(t->data.and.dxs); free(t->name); free(t); 
      continue;
    }
...
static int main_mpc(int argc, char **argv) {
...
int main(int argc, char **argv) {
  int (*volatile func)(int,char**) = main_mpc;
  tgc_start(&gc, &argc);
